    ...
    public class EntityUtitl
    {
        ...
        public static string DBInsert
        {
            get
            {
                ...
                int dataBaseType = 1;
                string dataBaseToExcel = ";SELECT @@IDENTITY;";
                int.TryParse(ConfigurationManager.AppSettings["DBType"], out dataBaseType);
                if (dataBaseType == 5)
                    dataBaseToExcel = "";
        ...
        public static string GuidType(Guid guid)
        {
            int dataBaseType = 1;
            int.TryParse(ConfigurationManager.AppSettings["DBType"], out dataBaseType);
            if (dataBaseType == 5)
        ...
        static void SetPropertyValue(object entity, string propertyName, object value)
        {
            ...
            if (property != null)
            {
                value = GetPropertyTypeValue(entity, value, property);
            }
            else
            {
        ...
        private static object GetPropertyTypeValue(object entity, object value, PropertyInfo property)
        {
            if (property.PropertyType.Name == "Guid")
            {
                // if(property.PropertyType
                //用索引的方式  在model中找到对应的property并赋值
                property.SetValue(entity, Convert.ChangeType(new Guid(value.ToString()), property.PropertyType), null);
            }
            else
            {
        ...
        public static string CreateSelectSql(string sql, IEntity entity)
        {
            ...
            if (sql.ToLower().StartsWith("select ") == false)
            {
                ...
                fields = '[' + entity.EntitySetName + DBType + "].[" + fields.Replace(",", "],[" + entity.EntitySetName + DBType + "].[") + "]";
                sql = "SELECT " + fields + (sql.ToLower().StartsWith("from") ? " " : (" FROM [" + entity.EntitySetName + DBType + "]" + (sql.ToLower().StartsWith("where") || sql.ToLower().StartsWith("order by") || sql.Length == 0 ? " " : " WHERE ") + sql));
            }
            return sql;
        }
        /// <summary>
        ...
        public static string CreateInsertSql(IEntity entity)
        {
           ...
           // fields = fields.Substring(fields.Length - 1, 1);
            ...
            string sql = string.Format("INSERT INTO [{0}] ({1}) VALUES({2}){3}",
                entity.EntitySetName + DBType,
                "[" + fields.Replace(",", "],[") + "]",
                "@" + fields.Replace(",", ",@"), DBInsert);
            return sql;
        }
