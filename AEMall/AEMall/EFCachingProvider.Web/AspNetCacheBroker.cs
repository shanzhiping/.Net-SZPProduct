...
namespace EFCachingProvider.Web
{
    ...
    /// </summary>
    public class AspNetCacheBroker : ICache
    {
        ...
        public AspNetCacheBroker(HttpContext httpContext)
        {
        ...
        public void PutItem(string key, object value, IEnumerable<string> dependentEntitySets, TimeSpan slidingExpiration, DateTime absoluteExpiration)
        {
            ...
            foreach (var entitySet in dependentEntitySets)
            {
                ...
                try
                {
                    aggDep.Add(new SqlCacheDependency("CacheDB", entitySet));
                }
                catch
                {

                    SqlCacheDependencyAdmin.EnableTableForNotifications(ConfigurationManager.ConnectionStrings["ConnectionString"].ConnectionString, entitySet);
                }
            }
            ...
            try
            {
                ...
                aggDep.Add(new CacheDependency(new string[0], dependentEntitySets.Select(c => DependentEntitySetPrefix + c).ToArray()));

                //SqlCacheDependency  cd = new SqlCacheDependency("CacheDB", dependentEntitySets.Select(c => DependentEntitySetPrefix + c).ToArray());
                cache.Insert(key, value, aggDep, absoluteExpiration, slidingExpiration, CacheItemPriority.Normal, null);
            }
            catch (Exception)
            {
