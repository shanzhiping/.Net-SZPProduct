...
namespace SZP.Core
{
    public abstract class  BaseDao<TEntity> : IDao<TEntity> where TEntity :IEntity 
    {
        ...
        static string DBType = EntityUtitl.DBType;
        /// <summary>
        ...
        static BaseDao()
        {
            ...
            connectionString = ConfigurationManager.ConnectionStrings["DBConn" + dataBaseType.ToString()].ConnectionString;
        ...
        public BaseDao()
        {
            if (dbType == DBHelp.DatabaseType.SqlServer)
            {
                ...
                transaction = new DBHelp.TransactionToSql();
            }
            switch (dbType)
            {
                case DBHelp.DatabaseType.SqlServer:
                    db = new DBHelp.SQLServer(connectionString);
                    transaction = new DBHelp.TransactionToSql();
                    break;
                case DBHelp.DatabaseType.Access:
                    db = new DBHelp.Access(connectionString);
                    transaction = new DBHelp.TransactionToAccess();
                    break;
                case DBHelp.DatabaseType.Excel:
                    db = new DBHelp.Excel(connectionString);
                    transaction = new DBHelp.TransactionToExcel();
                    break;
            }
        ...
        {
            TEntity entity = Activator.CreateInstance<TEntity>();
            string sql = "SELECT COUNT(1) FROM [" + entity.EntitySetName + DBType+"]";
        ...
        public virtual IDbDataParameter[] CreateParameters(TEntity entity, string sql)
        {
            ...
            foreach (Match m in mc)
            {
                ...
                if (prop != null)
                {
                    ...
                    //{
                    //    value = EntityUtitl.GuidType(new Guid(value.ToString()));
                    //}
